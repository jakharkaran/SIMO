...
%% 
function [out, AlgorithmBo, Algorithmca, BoMeanArray, errorInterfaceMeanArray] = SIMO(pp, BoStart, BoEnd)
% clear;
% close all;
...
% BoStart = -100;      % First assumed Bo value
% BoEnd = 10;       % Last assumed Bo value
% BoIncrement = -0.01; % Incremental/Decremental step in assumed value of Bo as it is incremented/decremented from BoStart to BoEnd
...
BoErrorArray = zeros(MBo,2);
errorInterfaceAssume = zeros(1,5);
counter = 0;
...
%%
        ...
        stdEqvCurvature(countIteration) = std(out(:,4) - out(:,2).*gravitational);
        ...
        if plt==1
            ...
            plot(1:countIteration, stdEqvCurvatureArray(countBo,1:countIteration), 'c', 'LineWidth',1);
            xlabel('Iteration #', 'Color', 'k', 'FontWeight', 'bold');
            ylabel('c_{v}     ', 'Rotation', 0, 'Color', 'k', 'FontWeight', 'bold');
            axis([0 countIteration 0 1]); 
            ...
            counter = counter + 1;
            if dt == dtLarge && countIteration>25
            ...
            if (BoAssume(5) - BoAssume(1) > 0.3) && (countIteration > 25)
                break;
            end
    end
    errorInterface = mean(errorInterfaceArray(countBo,...
        countIterationArray(countBo)-9:countIterationArray(countBo)));
    
    BoErrorArray(countBo,1) = Bo;
    BoErrorArray(countBo,2) = errorInterface;
    
    if BoAssumeCounter == 1
        errorInterfaceAssume(countBo) = errorInterface;
        
        if countBo == 5
            errorInterfaceAssumeOld = errorInterfaceAssume;
            [~, minInd] = min(errorInterfaceAssume);
            if minInd == 1
                minInd = 2;
            elseif minInd == 5
                minInd = 4;
            end
            errorInterfaceAssume(1) = errorInterfaceAssumeOld(minInd-1);
            errorInterfaceAssume(3) = errorInterfaceAssumeOld(minInd);
            errorInterfaceAssume(5) = errorInterfaceAssumeOld(minInd+1);
            errorInterfaceAssume(2) = 0;
            errorInterfaceAssume(4) = 0;
            
            BoAssumeOld = BoAssume;
            BoAssume(1) = BoAssumeOld(minInd - 1);
            BoAssume(3) = BoAssumeOld(minInd);
            BoAssume(5) = BoAssumeOld(minInd + 1);
    ...
    elseif BoAssumeCounter == 2
        errorInterfaceAssume(2) = errorInterface;
        BoAssumeCounter = 4; 
        
    elseif BoAssumeCounter == 4
        errorInterfaceAssume(4) = errorInterface;
        BoAssumeCounter = 2; 
    ...
    if ((BoAssume(5) - BoAssume(1)) < 0.001)
        break;
    end
